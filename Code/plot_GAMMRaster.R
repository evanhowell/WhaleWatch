plot_GAMMRaster <- function(predictvecvec) {
  ...
  plot(rasterupper,legend=T,legend.width=5,las=1,zlim=c(0,100),col=coltheme$regions$col,main='Upper',xlim=c(-134.6,-115),ylim=c(30,48.7))
  #plot(rasterupper, legend.only=T, legend.width=2)
  plot(wrld_simpl, add = T, col="light grey",border="dark grey")
  
  plot(rasterdensity,legend=T,legend.width=5,las=1,zlim=c(0,3),col=coltheme$regions$col,main='Average',xlim=c(-134.6,-115),ylim=c(30,48.7))
  plot(wrld_simpl, add = T, col="light grey",border="dark grey")
